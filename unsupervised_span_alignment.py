...
SPACY_NAMES = {"en": {"spacy_model_name": 'en_core_web_md', "parser_name": 'benepar_en3'},
               ...
               "de": {"spacy_model_name": 'de_core_news_md', "parser_name": 'benepar_de2'},
                          }
...
def parse_args():
        ...
        parser.add_argument('--gpu', action='store_true', help='use GPU for parser')
        parser.add_argument('--output-table', action='store_true', help='whether to output the alignment table')
...
def align(source_tokens, target_tokens, tokenizer, model, model_layer, span_extractor, decoding_method, source_span_boundaries=None, target_span_boundaries=None, source_lang="en", target_lang="fr", max_source_span_width=None, max_target_span_width=None, allow_overlap=False, couple=False, verbose=False, output_table=False):
        if verbose:
                print('='*20)
        ...
        if decoding_method == "greedy":
                ...
                raise NotImplementedError
        elif decoding_method == "weighted_symmetrized_softmax":
                used_source_spans = []
                used_target_spans = []
                result = []
...
def main():
        ...
        def _get_tokens_and_spans(data, lang, side, spacy_parsers=None):
                ...
                if not given_spans:
                        if args.span_enumeration == "parse":
                                ...
                                spans = [(s.start, s.end-1) for s in sent._.constituents]
        ...
        for item in data:
                ...
                if source_span_boundaries == []:
                        alignment = []
                else:
                        alignment, table = align(source_sentence, target_sentence, tokenizer, model, args.model_layer, span_extractor, args.decoding_method, source_span_boundaries, target_span_boundaries, max_source_span_width=args.max_source_span_width, max_target_span_width=args.max_target_span_width, allow_overlap=args.allow_overlap, couple=args.couple, verbose=args.verbose, output_table=args.output_table)

                if args.output_table:
                        outputs.append({"source_tokens": source_sentence, "target_tokens": target_sentence, "alignment": alignment, "table": table.tolist()})
                else:
